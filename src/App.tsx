...
export default function App() {
  ...
  const setupAudio = async () => {
    ...
    if (!audioCtxRef.current) {
      ...
      analyser.connect(ctx.destination)        // ← This makes the sound come out
  ...
  const drawVisualizer = () => {
    ...
    const loop = () => {
      ...
      // === All your visualizer modes (unchanged) ===
      if (currentViz === 'bars') {
        ...
        data.forEach((val, i) => {
          ...
          if (val > 145 && particlesRef.current.length < 400) {
            particlesRef.current.push({
              ...
              vy: -(Math.random() * 6 + 2),
              life: 1,
              color: `hsl(${Math.random() * 55}, 100%, 60%)`
            })
          }
        })
      }
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: s.panel, border: `1px solid ${s.panelBorder}`, padding: '12px 18px', display: 'flex', alignItems: 'center', gap: '14px', backdropFilter: 'blur(10px)' }}>
              <div style={{ display: 'flex', gap: '3px', alignItems: 'flex-end', height: '20px' }}>
                {[1, 0.6, 0.8, 0.4, 0.9].map((h, i) => (
                  <div key={i} style={{
                    width: '3px', background: s.accent, boxShadow: `0 0 6px ${s.accent}`,
                    height: isPlaying ? `${h * 20}px` : '4px',
            ...
            <div style={{ position: 'relative', border: `1px solid ${s.panelBorder}`, background: s.bgSolid, overflow: 'hidden' }}>
              {['top:0;left:0', 'top:0;right:0', 'bottom:0;left:0', 'bottom:0;right:0'].map((pos, i) => (
                <div key={i} style={{
                  ...
                  borderTop: i < 2 ? `2px solid ${s.accent}` : 'none',
                  borderBottom: i >= 2 ? `2px solid ${s.accent}` : 'none',
                  borderLeft: i % 2 === 0 ? `2px solid ${s.accent}` : 'none',
                  borderRight: i % 2 === 1 ? `2px solid ${s.accent}` : 'none',
                  zIndex: 2
                }} />
              ))}
            ...
            <div style={{ background: s.panel, border: `1px solid ${s.panelBorder}`, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '10px', backdropFilter: 'blur(10px)' }}>
              ...
              <button onClick={togglePlay} style={ctrlBtn(s, true)}>
                {isPlaying ? '⏸' : '▶'}
              </button>
          ...
          {/* Playlist Sidebar */}
          <div style={{ background: s.panel, border: `1px solid ${s.panelBorder}`, display: 'flex', flexDirection: 'column', backdropFilter: 'blur(10px)', maxHeight: '580px' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '6px' }}>
              {tracks.length === 0 ? (
                ...
                <div key={i} onClick={() => playTrack(i)} style={{
                  ...
                  <span style={{ fontFamily: "'Share Tech Mono', monospace", fontSize: '0.6rem', color: s.subtext, minWidth: '22px', textAlign: 'right' }}>
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <div style={{ flex: 1, overflow: 'hidden', fontSize: '0.82rem', color: i === currentIndex ? s.accent : s.text }}>
                    {track.name.replace(/\.[^/.]+$/, '')}
                  </div>
